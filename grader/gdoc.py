...
def authorize_and_return_worksheet() -> gspread.worksheet.Worksheet:
    creds = ServiceAccountCredentials.from_json_keyfile_name("creds.json", SCOPE)  # need full path
...
def update_ws_stats(worksheet: gspread.worksheet.Worksheet, name, stats):
    ...
    if name not in names:
        worksheet.update_cell(len_names + 1, 1, name)  # +1, because numeration in table starts from 1
        names.append(name)
