...
def test_all(descent=Adagrad):
    test_interface(descent)
    test_public(descent)
    test_default(descent)
    test_random(descent, 100)
    print('All tests passed!')
    _update_stats('gradient_descent', 'Adagrad')
    print_stats('gradient_descent')


def test_interface(descent=Adagrad):
...
def test_public(descent=Adagrad):
    with ExceptionInterception():
        desc = descent(np.array([[1.0], [1.0], [1.0]]), 1.5, eps=0)
        ...
        assert np.allclose(desc.W, np.array([[1.75], [0.25], [1.75]]), atol=10 ** -6), \
            'difference misuse noticed'


def test_default(descent=Adagrad):
    with ExceptionInterception():
        ...
        desc = descent(input_W, input_lambda, eps=0.01)
...
def test_random(descent=Adagrad, iterations=1):
    with ExceptionInterception():
        np.random.seed(214)
        for test in range(iterations):
            ...
            desc = descent(inp_w0, inp_lambda, eps=inp_eps)
