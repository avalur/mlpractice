...
def test_all(descent=AdagradReg):
    test_interface(descent)
    test_public(descent)
    test_default(descent)
    test_random(descent, 100)
    print('All tests passed!')
    _update_stats('gradient_descent', 'AdagradReg')
    print_stats('gradient_descent')


def test_interface(descent=AdagradReg):
...
def test_public(descent=AdagradReg):
    with ExceptionInterception():
        desc = descent(np.array([[1.0], [1.0], [1.0]]), 1.5, eps=0, mu=0.5)
        ...
        assert np.allclose(diff, np.array([[0.75], [0.75], [0.75]]), atol=10 ** -6), \
            'wrong difference calculated'
        assert np.allclose(desc.W, np.array([[0.25], [0.25], [0.25]]), atol=10 ** -6), \
            'difference misuse noticed'


def test_default(descent=AdagradReg):
    with ExceptionInterception():
        ...
        desc = descent(input_W, input_lambda, eps=0.01, mu=input_mu)
        ...
        expected_diff = (eta(154) / np.sqrt(0.01 + grad ** 2)) * grad
        ...
        g = 2 * (grad ** 2)
        diff_2 = desc.update_weights(grad, 155)
        expected_diff = (eta(155) / np.sqrt(0.01 + g)) * grad
        assert np.allclose(diff_2, expected_diff, atol=10 ** -6), \
            'wrong difference calculated'
        assert np.allclose(desc.W, input_W - diff - diff_2, atol=10 ** -6), \
            'difference misuse noticed'


def test_random(descent=AdagradReg, iterations=1):
    with ExceptionInterception():
        np.random.seed(214)
        for test in range(iterations):
            ...
            inp_eps = np.random.random()

            desc = descent(inp_w0, inp_lambda, eps=inp_eps, mu=inp_mu)
            ...
            expected_diff = (eta(inp_iteration) / np.sqrt(inp_eps + g)) * grad
            assert np.allclose(diff, expected_diff, atol=10 ** -6), \
                'wrong difference calculated'
            ...
            grad = desc.calc_gradient(inp_X, inp_Y)
            desc.calc_gradient(inp_X, inp_Y)
            expected_grad = 2 * np.dot(inp_X.T,
                                       (np.dot(inp_X,
            ...
            g += grad ** 2
            diff_2 = desc.update_weights(grad, inp_iteration + 1)
            expected_diff = (eta(inp_iteration + 1) / np.sqrt(inp_eps + g)) * grad
            assert np.allclose(diff_2, expected_diff, atol=10 ** -6), \
                'wrong difference calculated'
