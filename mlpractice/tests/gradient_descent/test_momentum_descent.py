...
def test_all(descent=MomentumDescent):
    test_interface(descent)
    test_public(descent)
    test_default(descent)
    test_random(descent, 100)
    print('All tests passed!')
    _update_stats('gradient_descent', 'MomentumDescent')
    print_stats('gradient_descent')


def test_interface(descent=MomentumDescent):
...
def test_public(descent=MomentumDescent):
    with ExceptionInterception():
        desc = descent(np.array([[1.0], [1.0], [1.0]]), 1.5, alpha=1)
        ...
        expected_grad = np.array([[-(3 + 1889.0 / 4608)], [-(9 + 35.0 / 96)], [-(1 + 33893.0 / 55296)]])
        assert np.allclose(grad, expected_grad, atol=10 ** -6), \
            'wrong gradient received'

        diff = desc.update_weights(grad, 3)
        expected_diff = 0.75 * np.array([[-(3 + 2273.0 / 4608)], [-(8 + 35.0 / 96)], [-(1 + 55013.0 / 55296)]])
...
def test_default(descent=MomentumDescent):
    with ExceptionInterception():
        ...
        expected_grad = 2 * np.dot(input_X.T,
                                   (np.dot(input_X,
                                           desc.W) - input_Y)) / input_X.shape[0]
...
def test_random(descent=MomentumDescent, iterations=1):
    with ExceptionInterception():
        np.random.seed(214)
