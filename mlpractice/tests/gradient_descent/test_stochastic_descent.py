...
def test_all(descent=StochasticDescent):
    test_interface(descent)
    test_public(descent)
    test_default(descent)
    test_random(descent, 100)
    print('All tests passed!')
    _update_stats('gradient_descent', 'StochasticDescent')
    print_stats('gradient_descent')


def test_interface(descent=StochasticDescent):
...
def test_public(descent=StochasticDescent):
    with ExceptionInterception():
        desc = descent(np.array([[1.0], [1.0], [1.0]]), 1.5, batch_size=2)
        ...
        expected_grad = np.array([[1 / 2], [2.0], [1 / 8]])
        assert np.allclose(grad, expected_grad, atol=10 ** -6), \
            'wrong gradient received'

        # update_weights
        diff = desc.update_weights(grad, 3)
        assert np.allclose(diff, np.array([[3 / 8], [3 / 2], [3 / 32]]), atol=10 ** -6), \
            'wrong difference calculated'
        assert np.allclose(desc.W, np.array([[5 / 8], [-1/ 2], [29 / 32]]), atol=10 ** -6), \
            'difference misuse noticed'


def test_default(descent=StochasticDescent):
    with ExceptionInterception():
        ...
        desc = descent(input_W, input_lambda, batch_size=3)
        np.random.seed(835)
...
def test_random(descent=StochasticDescent, iterations=1):
    with ExceptionInterception():
        np.random.seed(214)
        for test in range(iterations):
            ...
            desc = descent(inp_w0, inp_lambda, batch_size=inp_batch_size)
            np.random.seed(seed)
